...
def test_inheritance_w_new_data(identifiers, sources):
    ...
    composite_factory = Factory(
        data={"students": {"table": deepcopy(modified_table)}},
...
def test_multiple_inheritance(identifiers, sources):
    ...
    composite_factory = Factory(
        inherit_from=[base1_factory, base2_factory], sources=sources
    )
...
def test_merge_data():
    ...
    expected = {
        "students": {
            "table": "b",
            "dataframe": "b",
            "values": {"value_a1": "a", "X": "b", "value_b1": "b"},
        },
        "organizations": {"table": "b", "dataframe": "b"},
    }
