...
def compile_spec(search_path, scenario_selector=None, case_selector=None):
    ...
    for scenario in selected_scenario_cases:
        scenario['cases'] = list(
            filter(lambda case: re.compile(case_selector).search(case['case']), scenario['cases'])
        )
    ...
    spec['targets'] = [target for target in spec['targets'] if target['target'] in selected_targets]
